package beige_engine.graphics.voxels;
...
import beige_engine.graphics.voxels.VoxelRenderer.VoxelRendererParams;
...
import beige_engine.util.rlestorage.IntConverter.IntegerConverter;
import beige_engine.util.rlestorage.RLEArrayStorage;

public class VoxelModel {

    private static final Map<String, VoxelModel> MODEL_CACHE = new HashMap();

    public static VoxelModel load(String fileName) {
        if (!MODEL_CACHE.containsKey(fileName)) {
            VoxelModel s = new VoxelModel(fileName);
    ...
    private VoxelModel(String fileName) {
        RLEArrayStorage<Integer> colors = null;
        ...
        int[] finalColorPalette = colorPalette;
        ...
        params.voxelFaceToData = (vfi, dir) -> {
            ...
            float r = mod(colorHex, 256) / 255.f;
            float g = mod(colorHex >> 8, 256) / 255.f;
