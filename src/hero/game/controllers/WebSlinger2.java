    ...
    public void step() {
        if (controller.controller.buttonJustPressed(TRIGGER)) {
            ...
            for (int i = 0; i < NUM_JOINTS; i++) {
                ...
                wj.physics.velocity = controller.player.physics.velocity.add(controller.forwards().mul(100. * i / NUM_JOINTS));
                wj.physics.world = controller.player.physics.world;
                wj.create();
                joints.add(wj);
            }
            ...
            for (int iter = 0; iter < 100; iter++) {
                ...
                for (int i = 0; i < NUM_JOINTS - 1; i++) {
                    ...
                    if (dist > minDist) {
                        ...
                        wj.physics.applyForce(dir.mul(force), wj.pose.position);
                        wj.next.physics.applyForce(dir.mul(-force), wj.next.pose.position);
                    }

                    var smoothing = .01;
                    var velDiff = prevVels.get(i + 1).sub(prevVels.get(i));
                    wj.physics.applyImpulse(velDiff.mul(smoothing), wj.pose.position);
